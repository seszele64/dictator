"""E2E tests for the full dictation pipeline in whisper_dictate.toggle.
...
def e2e_env(tmp_path, monkeypatch, mock_config):
    ...
    monkeypatch.setattr(toggle, "bootstrap", lambda *a, **k: mock_config)

    # Mock the arecord subprocess (returns a process with a fake PID)
    popen_mock = Mock()
    popen_mock.return_value = Mock(pid=MOCK_PROCESS_PID)
    monkeypatch.setattr(toggle.subprocess, "Popen", popen_mock)
    ...
    toggle.AUDIO_FILE.write_bytes(WAV_HEADER)
...
class TestDictationPipelineE2E:
    ...
    def _start_stop(self, e2e_env: E2EEnv, mock_config) -> int:
        ...
        success, recording_id = toggle.stop_background_recording()
    ...
    def test_full_dictation_cycle(self, e2e_env, mock_config):
        ...
        assert toggle.PID_FILE.exists()
        assert toggle.STATE_FILE.exists()

        success, recording_id = toggle.stop_background_recording()
        assert success is True
        assert recording_id is not None
        assert not toggle.PID_FILE.exists()
        assert not toggle.STATE_FILE.exists()
        ...
        assert not toggle.AUDIO_FILE.exists()
    ...
    def test_start_sets_recording_state(self, e2e_env, mock_config):
        ...
        assert toggle.PID_FILE.exists()
        assert toggle.PID_FILE.read_text().strip() == str(MOCK_PROCESS_PID)
        assert toggle.STATE_FILE.exists()

        toggle.stop_background_recording()
