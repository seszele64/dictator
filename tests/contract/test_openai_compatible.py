"""Contract tests pinning the transcription provider seam (spec 008).

These tests exercise `OpenAICompatibleProvider` and the `create_transcriber`
factory against the `TranscriptionProvider` ABC contract: ABC conformance,
provider construction/properties, error wrapping, parameter forwarding, task
routing, and factory selection. The external OpenAI client is mocked; the
live-mode counterpart (real API, opt-in) lives in `test_live_provider.py`.

Pure domain-type unit tests (TranscriptionResult, TranscriptionError) stay in
...
class TestCreateTranscriber:
    ...
    def test_create_transcriber_groq(self):
        ...
        provider = create_transcriber(WhisperConfig(provider=WhisperProvider.GROQ.value, api_key="test-key"))
        assert isinstance(provider, OpenAICompatibleProvider)
        assert str(provider._client.base_url).startswith("https://api.groq.com/openai/v1")
