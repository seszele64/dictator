...
| ✅ | **P9 — CI matrix (Day 7)** | §7 | ✅ done (Day 7) |
...
| 8 | ✅ | P9 | Process | CI matrix 3.11/3.12/3.13 (astral-sh/setup-uv@v8+ enable-cache; `uv sync --frozen --extra dev`; pytest on matrix without `--cov`; separate coverage job on 3.12 w/ `check_coverage.py` + artefact; lint job ruff check + format check + mypy job; pip-audit step; drop pip-based lint) — **Done (Day 7)**: stale pip-era legacy workflow replaced by a uv-native SHA-pinned pipeline (setup-uv v10.0.1, superseding the spec's "v8+") — 5 jobs: lint (ruff check+format), typecheck (mypy strict), test matrix 3.11/3.12/3.13, coverage (py3.12: `--cov` + `check_coverage.py` floors + `coverage.json` artifact), audit (pip-audit on exported runtime deps); `uv sync --locked` so lock desync fails loudly (proven); 3.13 leg `allow-fail` as documented P10 bridge (green today — pydub only lazily imported + conftest-mocked; drop the bridge at P10); 2 transitional `--ignore-vuln` bridges (click PYSEC-2026-2132, idna PYSEC-2026-215 — fixed versions already in bounds; drop after next `uv lock --upgrade`); dependabot (deferred from P8) landed here: github-actions + uv ecosystems, weekly, grouped, 7d cooldown; first run caught a UV_PYTHON=3.x defect (fixed `97a6089`); PR #26 squash-merged | H | M | P7, P10 (for 3.13) | Day 7 |
...
| **7** | CI that means what it says | ✅ **done** — **P9** CI matrix + dependabot (uv-native, SHA-pinned) | ✅ Gate passed: PR #26 (run 33823452266) all-green on the PR branch — 659 passed / 4 skipped on all three legs incl. 3.13; per-module coverage floors + artifact; pip-audit clean; `uv sync --locked` desync fails loudly (proven); squash-merged; main push run green (33870109161) |
